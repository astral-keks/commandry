...
# Instructions
You are creating a **PowerShell MCP tool** in a form of script (`.ps1` files) or in a for of function (in `.psm1` modules) depending on the `tool form`. Follow these guidelines:
...
try {
    ...
    $result = @{
        ...
        Files = @($files | ForEach-Object {
            Write-Progress -Activity "Scanning" -Status "Processing file $($_.Name)"
            @{
                Name = $_.Name
                Size = $_.Length
    ...
    throw "Error scanning directory: $($_.Exception.Message)" # Or Write-Error "Failed to get file information: $($_.Exception.Message)"
}
```
